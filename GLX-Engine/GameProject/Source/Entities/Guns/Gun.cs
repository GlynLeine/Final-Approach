...
namespace GameProject
{
    ...
    abstract class Gun : BoundsObject
    {
        ...
        protected string m_reloadTexture;
        ...
        protected void Setup()
        {
            ...
            if (m_reloadTexture != null && m_reloadTexture != "")
                for (int i = 0; i < m_clipSize; i++)
                {
                    Sprite sprite = new Sprite(m_reloadTexture);
                    sprite.visible = false;

                    sprite.width = 70 / m_clipSize;
                    sprite.height = 20;
                    sprite.scale = (sprite.scaleX + sprite.scaleY) / 2f;

                    sprite.x = game.width / 2 + i * sprite.width - (m_player as Player).m_sprite.width / 2;
                    sprite.y = game.height / 2 + (m_player as Player).m_sprite.height / 2 + 8;

                    sprite.width = 50 / m_clipSize;
                    sprite.height = 20;
                    sprite.scale = (sprite.scaleX + sprite.scaleY) / 2f;
        ...
        public void SetActive(bool a_active)
        {
            ...
            if (!a_active)
                for (int i = 0; i < m_reloadBar.Count; i++)
                {
                    m_reloadBar[i].visible = false;
                }
        }
